...
Result<std::unique_ptr<Table>> InMemoryCatalog::UpdateTable(
  ...
  ICEBERG_ASSIGN_OR_RAISE(std::string location, CatalogUtil::WriteNewMetadataFile(
                                                    file_io_, base.get(), updated.get()));
  ICEBERG_RETURN_UNEXPECTED(
      root_namespace_->UpdateTableMetadataLocation(identifier, location));

  CatalogUtil::DeleteRemovedMetadataFiles(file_io_, base.get(), updated.get());

  return std::make_unique<Table>(identifier, std::move(updated), location, file_io_,
