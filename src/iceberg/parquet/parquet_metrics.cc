...
std::optional<Literal> ConvertParquetValue(std::shared_ptr<PrimitiveType> iceberg_type,
...
Result<FieldMetrics> CollectCounts(int32_t field_id,
  ...
  if (!column_idx.has_value()) {
    return InvalidArgument("No column found for field ID {}", field_id);
  }
  ...
  for (int rg = 0; rg < metadata.num_row_groups(); ++rg) {
    ...
    if (!column_chunk->is_stats_set()) {
      return InvalidArgument("No stats found for field ID {}", field_id);
    }

    auto stats = column_chunk->statistics();
    if (stats == nullptr || !stats->HasNullCount()) {
      return InvalidArgument("Null counts not set in statistics for field ID {}",
                             field_id);
    }
...
Result<FieldMetrics> CollectBounds(int32_t field_id,
                                   std::shared_ptr<PrimitiveType> iceberg_type,
                                   const ::parquet::SchemaDescriptor& parquet_schema,
                                   const ::parquet::FileMetaData& metadata) {
  auto column_idx = FindColumnIndex(parquet_schema, field_id);
  if (!column_idx.has_value()) {
  ...
  for (int32_t rg = 0; rg < metadata.num_row_groups(); ++rg) {
    ...
    if (!lower_bound.has_value() ||
        (converted_min.has_value() && converted_min.value() < lower_bound.value())) {
      lower_bound = std::move(converted_min);
    }
    ...
    if (!upper_bound.has_value() ||
        (converted_max.has_value() && converted_max.value() > upper_bound.value())) {
      upper_bound = std::move(converted_max);
    }
  }
...
Result<FieldMetrics> MetricsFromFieldMetrics(
  ...
  if (it == field_metrics.end()) {
    return NotFound("No pre-computed metrics for field ID {}", field_id);
  }
...
Result<FieldMetrics> MetricsFromFooter(int32_t field_id,
                                       std::shared_ptr<PrimitiveType> iceberg_type,
                                       const ::parquet::SchemaDescriptor& parquet_schema,
                                       const ::parquet::FileMetaData& metadata,
  ...
  auto column_idx = FindColumnIndex(parquet_schema, field_id);
  if (!column_idx.has_value()) {
    return NotFound("No Parquet column for field ID {}", field_id);
  }
  ...
  // Collect counts
  if (truncate_length <= 0) {
    return CollectCounts(field_id, parquet_schema, metadata);
  }

  // Collect bounds if truncate_length > 0
  return CollectBounds(field_id, iceberg_type, parquet_schema, metadata);
}
